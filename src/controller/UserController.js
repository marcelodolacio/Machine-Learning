export class UserController {
    ...
    async handlePurchaseAdded({ user, product }) {
        ...
        updatedUser.cars.push({
            ...product
        })

        await this.#userService.updateUser(updatedUser);

        const lastPurchase = updatedUser.cars[updatedUser.cars.length - 1];
    ...
    async handlePurchaseRemove({ userId, product }) {
        const user = await this.#userService.getUserById(userId);
        const index = user.cars.findIndex(item => item.id === product.id);

        if (index !== -1) {
            user.cars.splice(index, 1); // directly remove one item at the found index
    ...
    async displayUserDetails(user) {
        ...
        this.#userView.renderPastPurchases(user.cars);
